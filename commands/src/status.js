...
module.exports.use = async (message, args, index) => {
  ...
  .addField("STAFF", `${message.guild.owner.user.username}#${message.guild.owner.user.discriminator}`, true)
  .addField("Pais", region[message.guild.region], true)
  .addField("Usuarios", message.guild.memberCount, true)
  .addField("Miembros", message.guild.members.cache.filter(m => !m.user.bot).size, true)
